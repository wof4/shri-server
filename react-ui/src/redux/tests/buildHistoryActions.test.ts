import { buildListData, buildListDataUiType } from "../../types";
...
it('IS_SHOW_BUILD_FORM', () => {
    const action = { type: 'IS_SHOW_BUILD_FORM', payload: false } as const
...
it('SHOW_LOADER', () => {
    const action = { type: 'SHOW_LOADER', payload: false } as const
...
it('IS_ADD_NEW_BUILD', () => {
    const action = { type: 'IS_ADD_NEW_BUILD', payload: true } as const
...
it('SET_BUILD_LIST_DATA', () => {
    ...
    const action = { type: 'SET_BUILD_LIST_DATA', payload: data } as const
    // @ts-ignore
    expect(buildHistoryReducer(state, action)).toEqual({
        ...state,
