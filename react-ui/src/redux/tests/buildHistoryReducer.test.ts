...
it('getBuildsList -удачное получение билд листа', async () => {
    const buildApiMock = buildsApi as jest.Mocked<typeof buildsApi>
    buildApiMock.getBuildsList.mockImplementation(() => Promise.resolve(response));
...
it('setNewBuildTc -удачное отправка нового билда', async () => {
    ...
    const buildApiMock = buildsApi as jest.Mocked<typeof buildsApi>
    buildApiMock.postNewBuild.mockImplementation(() => Promise.resolve(response));
